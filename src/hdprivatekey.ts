 ...
 */
export const HDPrivateKey: any = (arg: any) => {
  const self: any = this;
  ...
  if (!(self instanceof HDPrivateKey)) {
    return new HDPrivateKey(arg);
  }
  if (!arg) {
    return self._generateRandomly();
  }

  if (Network.get(arg)) {
    return self._generateRandomly(arg);
  } else if (_.isString(arg) || BufferUtil.isBuffer(arg)) {
    if (HDPrivateKey.isValidSerialized(arg)) {
      self._buildFromSerialized(arg);
    } else if (JSUtil.isValidJSON(arg)) {
      self._buildFromJSON(arg);
    } else if (
      BufferUtil.isBuffer(arg) &&
      HDPrivateKey.isValidSerialized(arg.toString())
    ) {
      self._buildFromSerialized(arg.toString());
    } else {
      throw HDPrivateKey.getSerializedError(arg);
    }
  } else if (_.isObject(arg)) {
    self._buildFromObject(arg);
  } else {
 ...
 * @return {boolean}
 */
HDPrivateKey.isValidPath = function (arg: stringNumberType, hardened: boolean) {
  if (_.isString(arg)) {
 ...
 * @param {boolean?} hardened
 */
HDPrivateKey.prototype.deriveChild = function (
  arg: stringNumberType,
  hardened: boolean
) {
  if (_.isNumber(arg)) {
 ...
 * @param {boolean?} hardened
 */
HDPrivateKey.prototype.deriveNonCompliantChild = function (
  arg: stringNumberType,
  hardened: boolean
) {
  if (_.isNumber(arg)) {
...
HDPrivateKey.prototype._deriveFromString = function (
  path: string,
  nonCompliant: any
) {
  ...
  const derived = indexes.reduce(function (prev: any, index: number) {
 ...
 * @return {boolean}
 */
HDPrivateKey.isValidSerialized = function (
  data: dataType,
  network: networkType
) {
 ...
 */
HDPrivateKey.getSerializedError = function (
  data: dataType,
  network: networkType
) {
  /* jshint maxcomplexity: 10 */
...
HDPrivateKey._validateNetwork = function (
  data: dataType,
...
HDPrivateKey.prototype._buildFromJSON = function (arg: any) {
  return this._buildFromObject(JSON.parse(arg));
};

HDPrivateKey.prototype._buildFromObject = function (arg: any) {
...
HDPrivateKey.prototype._buildFromSerialized = function (arg: any) {
...
HDPrivateKey.prototype._generateRandomly = function (network: networkType) {
 ...
 */
HDPrivateKey.prototype._buildFromBuffers = function (arg: any) {
  /* jshint maxcomplexity: 8 */
  /* jshint maxstatements: 20 */
...
HDPrivateKey._validateBufferArguments = function (arg: any) {
  const checkBuffer = function (name: string, size: number) {
    const buff = arg[name];
 ...
 * @return {HDPrivateKey}
 */
HDPrivateKey.fromBuffer = function (arg: Buffer) {
